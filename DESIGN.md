...
- **Returns:**
  ```json
  {
    ...
    "composite":  0,
    "observation": { "temperature", "temperatureUnit", "windSpeed", "humidity", "conditions" },
    "elevation":   { "meters", "feet" },                  // null if USGS EPQS unavailable
    "floodZone":   { "zone", "description" },             // null if FEMA NFHL unavailable
    "state":      "FL",
    "climateZone": "1/2",
...
| §3.2 Weather Data Retrieval/Analysis  | Pull NWS data + display current conditions   | `/api/weather` proxies NWS point/forecast/alerts; the response's `observation` field (temp/temperatureUnit/windSpeed/humidity/conditions) renders as the "Current Conditions" card on the Overview tab. |
...
| §3.7 Site context (elevation, flood zone) | Surface engineering inputs architects use for site siting | `get_elevation_meters()` (USGS EPQS) + `get_flood_zone()` (FEMA NFHL layer 28) called from `/api/weather`; both best-effort with graceful `null` fallback. Sidebar renders elevation (ft / m) and flood zone (code + description); rows suppress when data unavailable. |
...
| 2026-05-07 | Brandon Stewart | v1.13 — SRS §3.7 (new section): site context — elevation + FEMA flood zone. Adds two best-effort upstream lookups to `/api/weather`: ground elevation via the USGS Elevation Point Query Service (`epqs.nationalmap.gov/v1/json`, free, no auth) and FEMA flood-zone designation via the National Flood Hazard Layer ArcGIS REST service (layer 28, S_FLD_HAZ_AR, also free + public). New helpers `get_elevation_meters()`, `get_flood_zone()`, `_build_elevation_payload()`, plus `_FLOOD_ZONE_DESCRIPTIONS` mapping FEMA zone codes (A, AE, VE, X, etc.) to human-facing meanings. Both calls use a tighter 5s timeout than the NWS path (8s) since values are decadal-scale and overwhelmingly cache-resident; on any failure the helper returns `None` and the response simply ships `elevation: null` / `floodZone: null` — never a 5xx. Two new sidebar rows on the frontend (`renderElevationRow`, `renderFloodZoneRow`) render the values when present and suppress entirely when null, so users never see "N/A" rows for missing data. New `.meta-aux` and `.meta-row-stacked` styles (with mobile breakpoint inheritance). Three new route tests (happy path with both upstreams responding, graceful 503 fallback to null, FEMA "no overlapping polygon → Zone X" informative answer); suite 81 → 84. New §7.2 fields, §13 traceability row for §3.7. SRS gains a new §3.7 "Site context" section with two use cases (UC1 retrieve, UC2 graceful degradation on upstream failure); SRS revision-history block notes the addition. |
| 2026-05-07 | Brandon Stewart | v1.12 — SRS §3.2 closeout: surface current observation in the Overview tab. The `/api/weather` response has always carried an `observation` object (temperature, windSpeed, humidity, conditions sourced from the first NWS forecast period), but the Overview tab silently dropped it — temperature, conditions, and wind never reached the user. New `renderObservation(obs)` in `frontend/app.js` adds a four-card "Current Conditions" row above the hazard snapshot grid; suppresses itself when `observation` is empty (e.g. NWS returned zero forecast periods) rather than rendering em-dashes. Backend gains `temperatureUnit` in the `observation` payload so the frontend doesn't have to assume Fahrenheit. New `.conditions-grid` / `.condition-card` styles in `styles.css` (responsive `auto-fit` minmax, collapses to 2 columns on the existing ≤960px breakpoint). Two new route tests lock the field shape (`temperature`, `temperatureUnit`, `windSpeed`, `humidity`, `conditions`) and the empty-periods suppression contract — suite 79 → 81. §7.2 endpoint spec updated to include `temperatureUnit`; §13 traceability for §3.2 expanded to cite the rendered card. No SRS amendment needed — this implements an existing §3.2 contract that had been satisfied at the API layer but never wired to the UI. |
